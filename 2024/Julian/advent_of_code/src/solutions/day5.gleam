...
fn sort_invalid_rows(rules: dict.Dict(Int, Rules), row: List(Int)) -> List(Int) {
    ...
    |> list.sort(fn(x, y) {
        ...
        let rules_for_sort = dict.get(rules, x) |> result.unwrap(Rules(set.new(), set.new()))

        case set.contains(rules_for_sort.before, y) {
            True -> order.Lt
            _ -> case set.contains(rules_for_sort.after, y) {
                True -> order.Gt
                _ -> order.Eq
            }
        }
...
fn is_valid_row(rules: dict.Dict(Int, Rules), row: List(Int)) -> Bool {
    ...
    |> list.index_map(fn(val, index) { 
        ...
        let val_rules = dict.get(rules, val) |> result.unwrap(Rules(set.new(), set.new()))

        set.is_disjoint(val_rules.after, before) && set.is_disjoint(val_rules.before, after)
    })
}
...
fn parse_rules(rules: List(String)) -> dict.Dict(Int, Rules) {
    ...
    |> list.fold(dict.new(), fn(acc, rule_row) {
        let #(key, value) = case string.split(rule_row, "|") {
            [key, value] -> #(key |> int.parse |> result.unwrap(0), value |> int.parse |> result.unwrap(0))
            _, -> #(0, 0)
        }
...
fn parse_input_row(row: List(String)) -> List(List(Int)) {
    row
    |> list.map(fn(x) {case string.split(x, ",") {
        [_, ..] as inputs -> inputs |> list.map(fn(b) {int.parse(b) |> result.unwrap(0)})
        _ -> []
    }})
}